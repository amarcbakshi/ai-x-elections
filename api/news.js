...
async function feedMode(q, state) {
  const query  = state ? `${q} ${state}` : q;
  const apiUrl = `https://content.guardianapis.com/search?q=${encodeURIComponent(query)}&api-key=test&show-fields=trailText&page-size=25&order-by=newest&section=us-news`;
  const body   = await get(apiUrl);
  const parsed = JSON.parse(body);
